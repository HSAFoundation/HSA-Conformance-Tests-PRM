...
private:
  ...
  TestSpec* CreateTestSpec(TestDesc& testDesc)
  {
    ...
    initialContext->Put("sample.brig", testDesc.getContainer());
...
{
  TestDataProvider::init(true, true, 0, CoreConfig::Get(context)->Wavesize(), 0, true, context->Opts()->IsSet("XtestFtzF16"));
