...
#define WAVESIZE         (TestDataProvider::getWavesize())
...
//          ----------------------------
//
//      See EmlBackend::getProvider for details.
//
// SRC
//
...
// NB: Instructions and types not supported yet by emulator are commented out
...
BEGIN_TEST_DATA
    ...
    INST(COMBINE)     SRC_TYPE(B32) SRCT(B32T.NEW(0, MAX_U32).ADD((b32_t)WAVESIZE));
                      SRC_TYPE(B64) SRCT(B64T.NEW(0, MAX_U64).ADD((b64_t)WAVESIZE));
...
//INST(LDC)         // NB: not testable
  ...
  INST(SHUFFLE)       TYPE(S16X2) SRCT( S16X2T.ITESTBITS2(s16x2), S16X2T.ITESTBITS2(s16x2), B32T.NEW(      0x0,      0x1,      0x2,      0x3).ADD(-1) );
                      ...
                      TYPE(S8X4)  SRCT( S8X4T .ITESTBITS2(s8x4),  S8X4T .ITESTBITS2(s8x4),  B32T.NEW(     0x00,     0x46,     0x93,     0x5A).ADD(-1) );
...
//INST(STOF)        // NB: not testable
    ...
    INST(LD)          TYPE(S8)   SRCT(  S8T.NEW (0, MAX_S8,  -1, MIN_S8 )   );
                      ...
                      TYPE(B128) SRCT(  B128T.NEW(b128(0xAA000000BBBB0000ULL, 0xCCCCCCCCFFFFFFFFULL), b128(0x8000000000000001ULL, 0x0000000DE0000000ULL)) );

    INST(ST)          TYPE(S8)   SRCT(0, S8T.NEW (0,       WAVESIZE),  S8T.NEW(1, MIN_S8)      );
                      TYPE(S16)  SRCT(0, S16T.NEW(-1,      WAVESIZE),  S16T.NEW(0, MIN_S16)    );
                      TYPE(S32)  SRCT(0, S32T.NEW(0,       WAVESIZE),  S32T.NEW(1, MIN_S32)    );
                      TYPE(S64)  SRCT(0, S64T.NEW(MAX_S64, WAVESIZE),  S64T.NEW(0, MIN_S64)    );
                      TYPE(U8)   SRCT(0, U8T.NEW (MAX_U8,  WAVESIZE),  U8T.NEW(0, MID_U8)      );
