...
using HSAIL_ASM::align2num;
using HSAIL_ASM::getMaxAlignment;
using HSAIL_ASM::getInputArg;
using HSAIL_ASM::type2name;
using HSAIL_ASM::bitType2uType;
using HSAIL_ASM::getBrigTypeNumBytes;
...
//   --------------------------------------------------------------
//        $s10 $d10 $q10    first  vector register  (REG_IDX_VEC)
//        $s11 $d11 $q11    second vector register  (REG_IDX_VEC + 1)
//        $s12 $d12 $q12    third  vector register  (REG_IDX_VEC + 2)
//        $s13 $d13 $q13    fourth vector register  (REG_IDX_VEC + 3)
//   --------------------------------------------------------------
//
...
private:
    ...
    static const uint64_t MIN_PRIVATE_REGION_SIZE = 64 * 1024ULL;
...
private:
    ...
    //==========================================================================
private:
    static const uint8_t GAPS_FILLING = 0x80;
...
public:
    ...
    bool beginTestGroup(BrigContext* ctx, string name)
    {
        ...
        createGlobalMemTestArray(); // Create a global array for testing memory access (if required)
    ...
    void beginTestCode() 
    {
        createLocalMemTestArray(); // Create a local array for testing memory access (if required)
    }
    ...
    void makeTestInst(Inst inst, unsigned tstIdx)
    {
        ...
        for (int i = provider->getFirstOperandIdx(); i <= provider->getLastOperandIdx(); i++)
        {
            ...
            // NB: some operands of test instruction 'inst' may be NULL (e.g. which refer symbols)
            //     so operand kind must be identified using testSample
    ...
    // This backend generates its own symbols so default symbols are not required
    bool genDefaultSymbols() { return false; }
...
private:
    ...
    void emitInitCode(unsigned tstIdx)
    {
        ...
        if (hasMemoryOperand())
        {
            ...
            if (useSegRegAddress())
            {
                emitCommentHeader("Load target segment address to register");
                loadSymAddr2Reg();
            }
            else if (useFlatAddress())
            {
                emitCommentHeader("Convert target segment address to flat address");
                convertSymAddr2Flat();
            }
        }
...
private:
    ...
    OperandRegister getAddrReg(unsigned size = 0) 
    {
        if (size == 0) size = getModelSize();
        return context->emitReg(size, REG_IDX_ADDR); 
    }
    ...
    OperandOperandList getOperandVector(unsigned tstIdx, unsigned idx)  // Create register vector for i-th operand of test instruction.
    {
        ...
        assert(regSize == 0 || regSize == 32 || regSize == 64 || regSize == 128);
...
private:
    ...
    // - for tests working with segments in private memory region, indexReg = tstIdx * bundleSize
    ...
    OperandRegister loadMemIndexReg(unsigned tstIdx, OperandRegister idxReg, unsigned memBundleSize)
    {
        ...
        if (isPrivateMemRegion()) // Private, spill, etc
        {
            unsigned addrSize = getRegSize(idxReg);
...
private:
    ...
    {
        return getInputArg(context->getCurrentSbr(), idx);
    }
...
private:
    ...
    OperandAddress getMemoryOperand()
    {
        for (int i = 0; i < testSample.operands().size(); ++i)
        {
            if (OperandAddress addr = testSample.operand(i)) return addr;
        }
        return OperandAddress();
    }

    unsigned getMemoryOperandIdx()
    {
        for (int i = 0; i < testSample.operands().size(); ++i)
        {
            if (OperandAddress addr = testSample.operand(i)) return i;
        }
        assert(false);
        return -1;
    }
    ...
    {
        return getMemoryOperand();
    }

    bool useFlatAddress()   { return getSegment(testSample) == BRIG_SEGMENT_FLAT; }
    bool useSegAddress()    { return getSegment(testSample) != BRIG_SEGMENT_FLAT; }
    bool useSegSymAddress() { return useSegAddress() && isSymbolicAddress(getMemoryOperand()); }
    bool useSegRegAddress() { return useSegAddress() && !isSymbolicAddress(getMemoryOperand()); }

    unsigned getMemTestArraySegment()    
    { 
        unsigned seg = getSegment(testSample);

        if (seg == BRIG_SEGMENT_FLAT) // actual segment is encoded in offset
        {
            assert(hasMemoryOperand());
        
            OperandAddress addr = getMemoryOperand();
            assert(addr);

            seg = decodeAddressSegment(addr);
            assert(seg != BRIG_SEGMENT_NONE);
        }

        return seg;
    }

    static bool isSymbolicAddress(OperandAddress addr)
    {
        assert(addr);
        return addr.symbol();
    }

    static unsigned decodeAddressSegment(OperandAddress addr)
    {
        assert(addr);

        switch(addr.offset()) // offset is used to encode actual segment
        {
        ...
        default:                    return BRIG_SEGMENT_NONE;
        }
    }

    void createGlobalMemTestArray() { createMemTestArray(false); }
    void createLocalMemTestArray()  { createMemTestArray(true);  }

    void createMemTestArray(bool isFuncScope)
    {
        if (hasMemoryOperand() && isFuncScope == isFunctionScopeMemSeg())
        {
            unsigned elemType   = getMemTestArrayDeclType();
            unsigned elemTypeSz = getBrigTypeNumBits(elemType);
            unsigned dim        = getMemTestArrayDeclDim();
            unsigned align      = getMemTestArrayDeclAlign();
            unsigned segment    = getMemTestArraySegment();
            const char* name    = getTestArrayName(isFuncScope);
    
            emitComment();
            emitComment("Array for testing memory access");
            //dumpMemoryProperties();

            assert(segment != BRIG_SEGMENT_NONE);
            ...
            if (isInitializableSeg())
            {
                // Simplify initialization of 'b' and 'f' types
                elemType = getUnsignedType(elemTypeSz);
                memTestArray.type() = elementType2arrayType(elemType);
                memTestArray.init() = memTestArrayInitializer(elemType);
            }
        }
    }

    OperandConstantBytes memTestArrayInitializer(unsigned declType)
    {
        assert(isInitializableSeg());

        b128_t value;
        ArbitraryData initData;
        
        const uint8_t filling = GAPS_FILLING;
        unsigned const byteSize = 1;

        unsigned operandIdx = getMemoryOperandIdx();
        unsigned vecSize = testGroup->getData(0).src[operandIdx].getDim();
        unsigned elemSize = getBrigTypeNumBytes(getMemDataElemType());

        unsigned startOffset = getMemTestArrayBundleOffset() / 8;
        unsigned bundleSize  = getMemTestArrayBundleSize() / 8;

        unsigned pos = 0;
        unsigned gapSize = startOffset;
        unsigned flatSize = testGroup->getFlatSize();

        for (unsigned flatIdx = 0; flatIdx < flatSize; ++flatIdx) 
        {
            for (unsigned i = 0; i < gapSize; ++i) 
            {
                initData.write(&filling, byteSize, pos);
                pos += byteSize;
            }
            ...
            {
                Val x = data.src[operandIdx][k];
                value.set(x.getAsB64(0), 0);
                value.set(x.getAsB64(1), 1);

                initData.write(&value, elemSize, pos);
                pos += elemSize;
            }

            gapSize = bundleSize - elemSize * vecSize;
            assert(gapSize >= 0);
        }

        return context->emitInitializer(initData, declType);
    }
    ...
    void copyMemTestArray(unsigned tstIdx, unsigned arrayIdx, bool isDst)
    {
        ...
        if (isPrivateMemRegion() || glbAddrSize != memAddrSize || dataBundleSize != memBundleSize)
        {
        ...
        for (unsigned i = 0; i < vectorDim; ++i)
        {
            if (isDst) // Initialize memory test array
            {
                for (unsigned m = 0; m < memDim; ++m)
                {
                    ...
                    OperandAddress memAddr  = getMemTestArraySymAddr(memIndexReg, dataElemSize, i, m + memBundleOffset);
    ...
    OperandAddress getMemTestArraySymAddr(OperandRegister idxReg, unsigned elemSize = 0, unsigned elemIdx = 0, unsigned offset = 0)
    {
        assert(memTestArray);
    ...
    //
    //     Element#0.0, Element#0.1, ... Element#0.M, Footer   // test 0
    //     Element#1.0, Element#1.1, ... Element#1.M, Footer   // test 1
    //     ...
    //     Element#N.0, Element#N.1, ... Element#N.M, Footer   // test N
    //
    // For scalar operations, M = 1.
    // In most cases each workitem executes exactly one test, but in some cases there
    // may be several (e.g. tests for instructions with immediate operands may have 
    // several tests with different immediate values).
    //
    // For vector operations, M is the number of vector elements.
    // Element type is specified by operation.
    //
    // Footer is an optional memory pad between test data necessary to ensure _minimum_ 
    // required alignment of next test as requested by operation being tested.
    //
    ...
    bool     isGroupMemSeg()             { return getMemTestArraySegment() == BRIG_SEGMENT_GROUP; }
    bool     isPrivateMemSeg()           { return getMemTestArraySegment() == BRIG_SEGMENT_PRIVATE; }
    bool     isGlobalMemSeg()            { return getMemTestArraySegment() == BRIG_SEGMENT_GLOBAL; }
    bool     isSpillMemSeg()             { return getMemTestArraySegment() == BRIG_SEGMENT_SPILL; }
    bool     isReadonlyMemSeg()          { return getMemTestArraySegment() == BRIG_SEGMENT_READONLY; }

    bool     isInitializableSeg()        { return isGlobalMemSeg() || isReadonlyMemSeg(); }

    bool     isPrivateMemRegion()        { return isPrivateMemSeg() || isSpillMemSeg(); }
    bool     isFunctionScopeMemSeg()     { return isSpillMemSeg(); }

    bool     testLdSt()                  { return testSample.opcode() == BRIG_OPCODE_LD || testSample.opcode() == BRIG_OPCODE_ST; }

    unsigned getRequiredMemAlignNum()    { InstMem inst = testSample; return inst? align2num(inst.align()) * 8: 0; }
    ...
    unsigned getMemTestArrayBundlesNum()     { return isPrivateMemRegion()? testGroup->getGroupSize() : testGroup->getFlatSize(); } // size in bytes
    unsigned getMemTestArrayBundleDataSize() { return getMemDataElemSize() * getMaxDim(); }
    unsigned getMemTestArrayBundleSize()     { return getMemTestArrayBundleDataSize() + getFooterSize(); }

    unsigned getMemTestArrayBundleDataAlignSize() // Allocate dummy vector element to ensure proper alignment of subsequent bundles
    { 
        return (testLdSt() && getMaxDim() == 3)? getMemDataElemSize() : 0;
    }
    ...
    { 
        if (testLdSt())
        {
            unsigned dataSize  = getMemTestArrayBundleDataSize();

            // Ensure proper alignment of subsequent bundles
            unsigned minAlignSize = getRequiredMemAlignNum() * 2;
            if (minAlignSize > dataSize) return minAlignSize - dataSize;

            return getMemTestArrayBundleDataAlignSize();
        }
        else
        {
            return 0;
    ...
    { 
        if (isGroupMemSeg())      return MIN_GROUP_SEGMENT_SIZE;
        if (isPrivateMemRegion()) return MIN_PRIVATE_REGION_SIZE;
    ...
    unsigned getMaxTotalTestNum()
    {
        ...
        if (testLdSt()) 
        {
            ...
            // Note that memory array defined in a private region is allocated per test group,
            // while arrays in other segments are allocated for all tests.
            if (isPrivateMemRegion()) // private, spill, etc
            {
                // For private region, memory overhead should be computed per test group (not per test),
    ...
    // (worst case alignment size)
    static unsigned getMaxSegmentOverhead()
    {
        return align2num(getMaxAlignment());
    }
    ...
    void dumpMemoryProperties()
    {
        ...
        { 
            ...
            emitComment();
            emitComment("    -- elem type:     " + std::string(type2name(getMemDataElemType())));
    ...
    OperandAddress getMemTestArrayAddr()
    {
        if (useSegSymAddress())                     // segment address: [array][1234]
        {
            return getMemTestArraySegSymAddr();
        }
        else if (useSegRegAddress())                // segment address: [$d0]
        {
            return getMemTestArraySegRegAddr();
        }
        else // flat address                        // flat address:    [$d0]
        {
            assert(useFlatAddress());
            return getMemTestArrayFlatAddr();
        }
    }

    OperandAddress getMemTestArraySegSymAddr()      // segment address: [array][1234]
    {
        assert(memTestArray);
        ...
        return getMemTestArraySymAddr(getIdxReg1(memAddrSize), 0, 0, memBundleOffset);
    }

    OperandAddress getMemTestArraySegRegAddr()      // segment address: [$d0]
    {
        assert(memTestArray);

        unsigned segSize = context->getSegAddrSize(getMemTestArraySegment());
        return context->emitAddrRef(getAddrReg(segSize));
    }

    OperandAddress getMemTestArrayFlatAddr()        // flat address:    [$d0]
    {
        assert(memTestArray);

        return context->emitAddrRef(getAddrReg());
    }

    void convertSymAddr2Flat()
    {
        assert(memTestArray);

        unsigned segSize = context->getSegAddrSize(getMemTestArraySegment());
        OperandRegister tmp = getAddrReg(segSize);
        context->emitLda(tmp, getMemTestArraySegSymAddr());

        if (isGroupMemSeg() || isPrivateMemSeg())
        {
            context->emitStoF(getAddrReg(), tmp, getMemTestArraySegment());
        }
        else
        {
            assert(isGlobalMemSeg());
        }
    }

    void loadSymAddr2Reg()
    {
        assert(memTestArray);
...
protected:
    ...
    const char* getTestArrayName(bool isLocal) { return isLocal? "%var0" : "&var0"; }
...
private:
    ...
    static bool testableInst(Inst inst)
    {
        assert(inst);
        ...
        {
            //if (instMem.type() == BRIG_TYPE_B128 && OperandOperandList(inst.operand(0))) return false;
    ...
    static bool testableOperands(Inst inst)
    {
        ...
        for (int i = 0; i < inst.operands().size(); ++i)
        {
            ...
            if (OperandAddress addr = operand)
            {
                ...
                {
                    return !isOpaqueType(addr.symbol().elementType()) && !addr.reg() && addr.offset() == 0;
                }
                else if (addr.reg()) // segment id is passed via offset
                {
                    return isValidSegment(inst, decodeAddressSegment(addr));
                }
                else
                {
                    return false;
    ...
    // Check if operand segment is compatible with instruction inst
    static bool isValidSegment(Inst inst, unsigned operandSegment)
    {
        assert(inst);

        unsigned instSegment = getSegment(inst);

        assert(instSegment != BRIG_SEGMENT_NONE);
        assert(operandSegment != BRIG_SEGMENT_NONE);

        if (!isSupportedSegment(instSegment)) return false;
        if (!isSupportedSegment(operandSegment)) return false;

        if (instSegment == BRIG_SEGMENT_FLAT) // flat addressing
        {
            if (InstAtomic(inst)) // atomic segments are limited to global and group
            {
                return operandSegment == BRIG_SEGMENT_GLOBAL ||
                       operandSegment == BRIG_SEGMENT_GROUP;
            }
            else // only global, group and private segments may be mapped to flat
            {
                return operandSegment == BRIG_SEGMENT_GLOBAL ||
                       operandSegment == BRIG_SEGMENT_GROUP  ||
                       operandSegment == BRIG_SEGMENT_PRIVATE;
            }
        }
        else // segment addressing
        {
            return instSegment == operandSegment;
        }
    }
