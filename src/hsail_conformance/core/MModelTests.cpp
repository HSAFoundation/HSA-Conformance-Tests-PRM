...
protected:
    static const BrigType RES_TYPE = BRIG_TYPE_U32;
    static const unsigned RES_VAL_FAILED = 0;
    static const unsigned RES_VAL_PASSED = 1;

protected:
    static const BrigType WG_COMPLETE_TYPE = BRIG_TYPE_U32;
...
public:
        ...
        testComplete(0)
    {
        ...
        BrigType type_0 = type_1;
        BrigAtomicOperation atomicOp_1 = atomicOp_0;
        mapFlat2Group = isBitType(type_1);
        bool isPlainOp_1 = false;
    ...
    ItemList AtomicOperands(unsigned idx)
    {
        ...
        OperandAddress target = TargetAddr(LoadArrayAddr(idx), ArrayIndex(idx, 0));
    ...
    TypedReg AtomicLd(unsigned idx)
    {
        ...
        ItemList operands;
        TypedReg atomicDst = be.AddTReg(getUnsignedType(getBrigTypeNumBits(type[idx])));
        OperandAddress target = TargetAddr(LoadArrayAddr(idx), ArrayIndex(idx, 1));
    ...
    // ========================================================================
    // This is a test for memory model.
    //
    // The purpose of this test is to check pairs of write-write instructions
    // which should not be reordered.
    //
    // Each workitem in this test does the following:
    //  1. prepare test data at index wi.id:
    //      - write test data X to array0 at index wi.id
    //      - write test data Y to array1 at index wi.id
    //  2. read test data written by another workitem at another index:
    //      - read test data B from array1 at index testId
    //      - read test data A from array0 at index testId
    ...
    //  2. WGROUP kind.
    //     Items withing a wave test data written by other items within the same workgroup by in another wave.
    //  3. AGENT kind.
    ...
    //        
    //        testArray0[wi.id] = ExpectedValue(0);
    //        testArray1[wi.id] = ExpectedValue(1);
    //        
    ...
    //        
    //        private bool flag1 = ATOMIC_LD_AR(testArray1[testId]) == ExpectedValue(1);
    //        private bool flag0 = testArray0[testId] == ExpectedValue(0);
    //        ok[wi.id] = (flag0 && flag1)? PASSED : FAILED;
    //    }
    // 
    // --------------------------------------------------------------------------------
    ...
    //        do
    //        {
    //            private bool flag1 = (testArray1[testId] == ExpectedValue(1));
    //            private bool flag0 = (testArray0[testId] == ExpectedValue(0));
    //            ok[id] |= (!testComplete && found2 && found1)? PASSED : FAILED;
    //            testComplete |= flag1;
    //        }
    ...
    void KernelCode()
    {
        ...
        bool isAgentTest = (testKind == TEST_KIND_AGENT);
//isAgentTest = false;
        LoadArrayAddr(0);
        LoadArrayAddr(1);
        LoadResAddr();
        if (isAgentTest) LoadWgCompleteAddr();
        ...
        InitLoop(isAgentTest); // conditional
        ...
        if (isPlainOp[0]) PlainSt(0); else AtomicSt(0);
        if (FenceRequired()) Fence();
        if (isPlainOp[1]) PlainSt(1); else AtomicSt(1);

        if (testKind == TEST_KIND_WAVE || testKind == TEST_KIND_WGROUP) // 
        {
            Comment("make sure all workitems within a workgroup have completed writing test data");
    ...
    {
        OperandAddress target = TargetAddr(LoadResAddr(), ResIndex());
    ...
    void InitLoop(bool loopEnabled)
    {
        if (loopEnabled)
        {
            Comment("Clear 'testComplete' flag");

            testComplete = be.AddTReg(BRIG_TYPE_B1);
            be.EmitMov(testComplete, be.Immed(BRIG_TYPE_B1, 0));
        }
    }
    
    void StartLoop(bool loopEnabled)
    {
        if (loopEnabled)
        {
            be.EmitLabel("@LoopStart");
        }
    }

    void EndLoop(bool loopEnabled, TypedReg isFlagSet)
    {
        if (loopEnabled)
        {
            Comment("Update 'testComplete' flag");
            Or(testComplete, testComplete, isFlagSet);
    ...
    TypedReg LdWgComplete()
    {
        ...
        OperandAddress target = TargetAddr(LoadWgCompleteAddr(), idx);
    ...
    void IncWgComplete()
    {
        ...
        OperandAddress target = TargetAddr(LoadWgCompleteAddr(), Add(id, 1));
    ...
    bool IsValid() const
    {
        ...
        //if(!isValidTestOrder(memoryOrder[0], memoryOrder[1])) return false;
...
void MModelTests::Iterate(hexl::TestSpecIterator& it)
{
    ...
    TestForEach<MModelTest>(ap, it, "mmodel", 
                            cc->Grids().MModelSet(), 
                            cc->Memory().LimitedAtomics(),      //cc->Memory().AllAtomics(), 
                            cc->Segments().Atomic(),        cc->Segments().Atomic(), 
                            cc->Memory().AllMemoryOrders(), cc->Memory().AllMemoryOrders(), 
                            cc->Memory().AllMemoryScopes(), cc->Memory().AllMemoryScopes(), 
                            cc->Types().MemModel(),           //cc->Types().Atomic(), 
                            Bools::All()
                            );
}
...
void MModelTests::Iterate(hexl::TestSpecIterator& it)
{
    ...
    TestForEach<MModelTest>(ap, it, "mmodel", 
                            cc->Grids().MModelSet(), 
                            cc->Memory().AllAtomics(),      cc->Memory().AllAtomics(), 
                            cc->Segments().Atomic(),        cc->Segments().Atomic(), 
                            cc->Memory().AllMemoryOrders(), cc->Memory().AllMemoryOrders(), 
                            cc->Memory().AllMemoryScopes(), cc->Memory().AllMemoryScopes(), 
                            cc->Types().Atomic(),           cc->Types().Atomic(), 
