...
public:
  ...
  Value ExpectedResult() const {
    switch (atomicOp) {
      ...
      case BRIG_ATOMIC_ADD:     return Value(MV_UINT32, U32(workgroupSizeX * uint32_t(expectedValue)));
      case BRIG_ATOMIC_AND:
      case BRIG_ATOMIC_WRAPDEC: return Value(MV_UINT32, U32(0));
      case BRIG_ATOMIC_MAX:     return Value(MV_UINT32, U32(workgroupSizeX-1));
      case BRIG_ATOMIC_OR:
      case BRIG_ATOMIC_XOR:     return Value(MV_UINT32, U32(~(-1 << int32_t(workgroupSizeX/te->CoreCfg()->Wavesize()))));
  ...
  void EmulateAtomicOperation() {
    switch (atomicOp) {
      ...
      case BRIG_ATOMIC_MIN:     initialValue = workgroupSizeX + 1; immSrc0 = workgroupSizeX; break;
      case BRIG_ATOMIC_SUB:     initialValue = workgroupSizeX + 1; break;
      case BRIG_ATOMIC_WRAPINC: initialValue = 0;                  immSrc0 = workgroupSizeX; break;
      case BRIG_ATOMIC_WRAPDEC: initialValue = workgroupSizeX;     immSrc0 = workgroupSizeX + 1; break;
      default: break;
    }
  ...
  TypedReg Result() {
    ...
    if (BRIG_ATOMIC_MIN != atomicOp && BRIG_ATOMIC_MAX != atomicOp)
...
void BarrierTests::Iterate(hexl::TestSpecIterator& it) {
  ...
  Arena* ap = cc->Ap();
  TestForEach<BarrierTest>(ap, it, "barrier/atomics", cc->Grids().SeveralWavesSet(), cc->Memory().AllAtomics(), cc->Segments().Atomic(), cc->Memory().AllMemoryOrders(), cc->Memory().AllMemoryScopes(), Bools::All(), Bools::All());
}
